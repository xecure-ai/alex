...
def handle_missing_instruments(job_id: str, db) -> None:
    ...
    if missing:
        logger.info(f"Planner: Found {len(missing)} instruments needing classification: {[m['symbol'] for m in missing]}")
        ...
        try:
            ...
            if isinstance(result, dict) and 'statusCode' in result:
                if result['statusCode'] == 200:
                    ...
                    logger.error(f"Planner: InstrumentTagger failed with status {result['statusCode']}")
...
async def finalize_job_internal(job_id: str, summary: str, key_findings: List[str], 
                               recommendations: List[str], db) -> str:
    """
...
def create_agent(job_id: str, portfolio_data: Dict[str, Any], db):
    ...
    if model_id.startswith('anthropic.') and not model_id.startswith('us.'):
